...
function AddOrder() {
  ...
  const handleItemSelect = (id, selectedId) => {
    ...
    setItems(items.map(item => 
      item.id === id ? { ...item, itemId: selectedId, price: product ? product.itemprice : 0 } : item
    ));
  };
      ...
      <div className="row">
          ...
          <table className="table item-table">
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={item.id}>
                  <td><button className="btn btn-sm btn-danger" onClick={() => deleteItem(item.id)}><i className="fa-solid fa-trash"></i></button></td>
                  <td>{index + 1}</td>
                  <td>
                    <select className="form-control form-select-sm" value={item.itemId} onChange={(e) => handleItemSelect(item.id, e.target.value)}>
                      <option value="">Select Item</option>
                      {availableItems.map(ai => <option key={ai._id} value={ai._id}>{ai.itemname}</option>)}
                    </select>
                  </td>
        ...
        <div className="col-12 mt-3 p-3" style={{ backgroundColor: "whitesmoke" }}>
          ...
          <div className="d-flex justify-content-between align-items-center mt-3">
            ...
            <input type="number" className="form-control w-50 w-md-25" value={paidAmount} onChange={(e)=>setPaidAmount(Number(e.target.value))} />
          </div>
        ...
        <div className="col-12 mt-4 mb-5">
          <div className="row g-2">
            ...
            <div className="col-6">
              {/* Added onClick={handleSubmit} here */}
              <button 
